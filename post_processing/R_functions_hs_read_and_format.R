...
read.flux.outputs <-function(output_dir, sim_name, hs8=FALSE) {
  ...
  flux.l$Datetime <-round(as.POSIXct('1899-12-30')+(flux.l$Datetime*60*60*24)-3600,"mins")
  #flux.l$Datetime <-as.POSIXct(round(as.POSIXct('1899-12-30')+(flux.l$Datetime*60*60*24)-3600,"mins"),tz="")
  
  flux.l$Date <- format(flux.l$Datetime,"%m/%d/%Y")
  flux.l$hour <- flux.l$Datetime$hour
  
  return(flux.l)
  
}

read.solar.flux.outputs <-function(output_dir, sim_name, hs8=FALSE) {
...
read.hs.outputs <- function(output_dir, file_name, constituent_name, sim_name, hs_ver=9, sheet_name=NULL) {
  # Reads any output from heat source version 7-9. Does some formatting, and returns the data
  # as a dataframe in long format. Simulation name, constituent, string date and int
  # hour are added as ID variables.
  ...
  # Convert data from wide to long
  data.l <- melt(data.raw, id.vars =c("Datetime","sim", "constituent"),variable.name=c("Stream_km"))
  colnames(data.l) <- c("Datetime","sim", "constituent","Stream_km","value")
  
  data.l$Stream_km <- as.numeric(gsub(pattern="X", replacement="", 
                                      data.l$Stream_km, ignore.case = FALSE,fixed = FALSE))
  
  data.l$Datetime <-round(as.POSIXct('1899-12-30')+(data.l$Datetime*60*60*24)-3600,"mins")
  #data.l$Datetime <-as.POSIXct(round(as.POSIXct('1899-12-30')+(data.l$Datetime*60*60*24)-3600,"mins"),tz="")
...
calc.7dadm <- function(df) {
  ...
  # by date in long format. constituents names are added as an ID variable.
  ...
  colnames(data.l) <- c("Date","sim","Stream_km","constituent","value")
  
  # make sure it is sorted
  data.l <- data.l[with(data.l, order(sim,constituent,-Stream_km,Date)), ]
  
  # rename the constituents
  data.l$constituent <- gsub(pattern="max", replacement="Daily Maximum Temperature", 
                             data.l$constituent, ignore.case = FALSE, fixed = FALSE)
  
  data.l$constituent <- gsub(pattern="sdadm", replacement="7DADM Temperature", 
                             data.l$constituent, ignore.case = FALSE,fixed = FALSE)
...
calc.7dadm2 <- function(output_dir, sim_name, hs_ver=9, file_name="Temp_H2O", sheet_name=NULL) {
  ...
  # Function does some formatting, and returns a dataframe of the daily maximum and 7DADM temperatures 
  # by date in long format. Simulation name and constituents are added as an ID variable.
  ...
  data.raw <- read.hs.outputs(output_dir=output_dir, 
                              file_name=file_name, 
                              constituent_name="Hourly Temperature", 
                              sim_name=sim_name, 
                              hs_ver=hs_ver,
...
calc.summary <- function(df) {
  # Function to calculate maximum, median, and minimum values for all dates by sim, constituent, and stream_km,
  ...
  data.summary <- as.data.frame(as.list(aggregate(df$value, by=c(list(df$Stream_km), 
                                                  ...
                                                  FUN = function(x) c(mn = min(x,na.rm=TRUE),
                                                                      ...
                                                                      mx = max(x,na.rm=TRUE)))))
  # fix colnames
  colnames(data.summary) <- c("Stream_km", "sim", "constituent", "min", "median","max")
